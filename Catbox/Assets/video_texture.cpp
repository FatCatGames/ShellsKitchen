...
public:
  ...
  bool UpdateFromIYUV(const uint8_t* data, size_t data_size)
  {
      ...
      HRESULT hr = DX11::Context->Map(target_texture->GetTex().Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &ms);
      if (FAILED(hr))
          return false;
     //assert(format == DXGI_FORMAT_R8_UNORM);
      assert(data_size == width * height2 * 3 / 4);
      ...
      // Copy the Y lines
      uint32_t nlines = height2 / 2;
      uint32_t bytes_per_row = width * bytes_per_texel;
      ...
      DX11::Context->Unmap(target_texture->GetTex().Get(), 0);
      return true;
  }
  ...
  bool open(const char* filename) {
   ...
   /* CHECK_HR(MFTRegisterLocalByCLSID(
    ...
    CHECK_HR(pVideoReaderAttributes->SetUINT32(MF_SOURCE_READER_ENABLE_ADVANCED_VIDEO_PROCESSING, 1),
  ...
  void update( float elapsed ) {
    ...
    if (pVideoSample)
    {
      ...
      if (!target_texture) {
        ...
        height2 = height * 2;
        target_texture->CreateVideoTexture(DXGI_FORMAT_R8_UNORM, width, height2, 1, true);
      }
    
      UpdateFromIYUV(byteBuffer, buffCurrLen);
...
void VideoTexture::ShouldLoop(bool aFlag)
{
    internal_data->autoloop = aFlag;
}

void VideoTexture::ResetVideo()
{
    destroy();
    create(myFileName);
}

float VideoTexture::GetVideoLength()
{
    if (internal_data) 
    {
        return internal_data->video_time;
    }
    return 0.0f;
}
