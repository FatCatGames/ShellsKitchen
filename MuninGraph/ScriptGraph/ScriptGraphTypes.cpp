...
ImVec4 ScriptGraphDataTypeRegistry::GetDataTypeColor(const std::type_index aType)
{
	if(auto type = GetType(aType))
	{
		return type->Color;
...
ImVec4 ScriptGraphDataTypeRegistry::GetDataTypeColorNormalized(const std::type_index aType)
{
	return GetDataTypeColor(aType) / 255.0f;
}
