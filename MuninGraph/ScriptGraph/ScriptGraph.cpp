...
const ScriptGraphPin& ScriptGraphInternal::GetDataSourcePin(size_t aPinUID) const
{
	const ScriptGraphPin& dataPin = Super::GetDataSourcePin(aPinUID);
...
bool ScriptGraphInternal::Tick(float aDeltaTime)
{
	if (bShouldTick)
	{
		if (const auto it = myEntryPoints.find("Tick"); it != myEntryPoints.end())
		{
			...
			return RunWithPayload("Tick", tickPayload);
		}
	}
	return false;
}
