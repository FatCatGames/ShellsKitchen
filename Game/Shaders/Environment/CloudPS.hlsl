...
PixelOutput main(VertexToPixel input)
{
    ...
    result.Color.a = GetLinDepth(float4(input.WorldPosition.x, input.WorldPosition.y, input.WorldPosition.z, 1), FB_ToView, FB_ToProjection, MB_CustomFloat4.x, MB_CustomFloat4.y);
    
    //if (OB_AliveTime > OB_CustomFloat)
    //{
    float funTime = saturate(OB_CustomFloat - OB_AliveTime);
    //result.Color.rgb = albedoTexture.Sample(defaultSampler, input.AlbedoUV).rgb * MB_Color.rgb * MB_EmissiveStrength ;

    //}
   //if (funTime > 0.f)
   //{
    //result.Color.a = saturate(OB_CustomFloat);
   //}
   //else
   //{
   //     result.Color.a = saturate(OB_AliveTime - OB_CustomFloat); //easeInCubic(min(OB_AliveTime * OB_AliveTime, 1.f));
   //}
