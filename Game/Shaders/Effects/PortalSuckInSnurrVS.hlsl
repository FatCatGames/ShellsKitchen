...
VertexToPixel main(VertexInput input)
{
    ...
    float sinAngle = sin(-FB_TotalTime * 30.f);
    float cosAngle = cos(-FB_TotalTime * 30.f);
    ...
    float4 pos = float4(input.Position.x + OB_AliveTime, input.Position.y, input.Position.z + OB_AliveTime, input.Position.w);
