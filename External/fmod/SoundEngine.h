 ...
 * ----------------------------
 * By Daniel Borgshammar @ The Game Assembly
 *
...
public:
	 ...
	 */
	static unsigned int PlaySound(const std::string& aFileName, const float aVolume = 1.0f);
	 ...
	 * @returns The channel number of this sound.
	 */
	static unsigned int PlaySoundAtLocation(const std::string& aFileName, const float aVolume = 1.0f, std::array<float, 3> aPosition = {0, 0, 0});
	...
	static bool SetEventPaused(const SoundEventInstanceHandle& aHandle, bool aPaused);

	static bool IsEventPlaying(const SoundEventInstanceHandle& aHandle);

	static bool EventRelease(const SoundEventInstanceHandle& aHandle);

	static bool GetBusPaused(const std::string& aBusName, bool* aPausedState);

	static bool SetBusPaused(const std::string& aBusName, bool* aPausedState);

	static bool GetBusVolume(const std::string& aBusName, float* aVolume);

	static bool SetBusVolume(const std::string& aBusName, float aVolume);

	static bool StopAllEvents(const std::string& aBusName, FMOD_STUDIO_STOP_MODE aMode);

	static ListenerHandle GetListener(int aListener);
};